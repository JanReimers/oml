...
class VecLimits
{
 public:
  ...
  VecLimits(subsc_t,subsc_t); //!<Construct from lower and upper bounds.
  //@}
 ~VecLimits();

  static index_t size(subsc_t,subsc_t)      ;
  //! Returns number of elements.
         index_t size(               ) const;

  index_t Offset         (subsc_t) const;
  bool    Check          (       ) const;
  bool    CheckIndex     (subsc_t) const;
  //! Comparison.
  ...
  */
  ...
  subsc_t Low;   //!<lower std::vector index limit.
  subsc_t High;  //!<upper std::vector index limit.
};
...
inline VecLimits::VecLimits(subsc_t low, subsc_t high) : //COnstruct from lower and upper bounds.
  Low(low),
  High(high) 
  {
    CHECK;
...
//
inline index_t VecLimits::size(subsc_t low, subsc_t high) 
{
...
inline index_t VecLimits::Offset(subsc_t i) const 
{
  return i-Low;
}
